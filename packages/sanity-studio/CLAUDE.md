...
5. [ ] **Guide entry (required for every editor-authored type):** add a `guideContent['<type>']` entry in `src/inspectors/guide-content.ts` with `intro` (intention), `placement` (where it surfaces on the site), and optional `tips` (config guidance for non-obvious choices). The guide panel then appears automatically at the top of the document form (always visible, no toggle — via the global `GuideFormInput` registered in `form.components.input`). It is a pure curation aid — it does **not** show a publish-readiness list; Sanity's native validation owns "what's still outstanding". Match the `responsibility` reference; plain Dutch. Skip only for system/non-editor types (e.g. `searchFeedback`).
...
8. [ ] `pnpm turbo run lint type-check test build --filter=...@kcvv/sanity-schemas` passes — the leading `...` checks everything that depends on the schemas: this package, both studios, typegen and the web build.
