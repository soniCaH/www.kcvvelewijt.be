...
**Surface:** `<EditorialHero>` — single component with 7 variants (`transfer | match-preview | interview | event | announcement | generic | player`). Variant changes only the right-column artefact + kicker labels; headline + intro + CTA structure is shared.
**Issue:** #1525 · master design `docs/plans/2026-04-27-redesign-master-design.md` §4.4 / §5
...
|---|---|---|
| A | `option-a-asymmetric-broadsheet.html` | Asymmetric broadsheet — 60/40 left/right grid; vertical rule between text and artefact column. |
...
## Variant artefact map

| Variant | A — Broadsheet | B — Poster | C — Cover frame |
|---|---|---|---|
...
| `match-preview` | TapedCard with opponent shield + date/venue/competition column | Wide band: shield + flat list + tickets CTA | TapedCard breaks frame; date/venue strip outside frame |
| `interview` | TapedCard with two PlayerFigure thumbnails | 2-up band of PlayerFigure thumbnails | Two TapedCard PlayerFigure thumbnails breaking frame at angles |
| `event` | TapedCard event-poster crop | Full-width poster crop band + ticket-stub date marker | Event poster TapedCard with TicketStub punched through frame |
...
| `generic` | Typographic mark on TapedCard | Drop-cap + TapeStrip ornament | Typographic ornament inside frame, body type takes weight |
| `player` | TapedCard with PlayerFigure + TicketStub + MonoLabel "NIEUW" | PlayerFigure right-of-centre with negative space left | PlayerFigure on TapedCard rotation B + TicketStub punched through bottom frame edge |
