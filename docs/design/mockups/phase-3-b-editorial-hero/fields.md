...
Every EditorialHero variant has access to these. Two rows reflect **post-migration shape** because the locked decisions in Ask 8 and Ask 9 below change the schema (`coverImage` becomes required; `title` converts from string to constrained Portable Text + Accent decorator). Both are blocking migrations for `3.B.2`; until they ship, treat the "current" notes as the live schema and the "target" notes as the contract `<EditorialHero>` is being designed against.

| Field             | Type                                                                                         | Required?                                                  | UI use                                                                |
| ----------------- | -------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | --------------------------------------------------------------------- |
| `articleType`     | enum                                                                                         | ✅                                                         | discriminator → variant + kicker first label                          |
| `title`           | **target:** PT (single block, Accent decorator only) — _current:_ `string` — see Ask 9 below | ✅                                                         | headline (PT-aware rendering or `serializeTitle()` for non-display)   |
| `slug`            | slug                                                                                         | ✅                                                         | route                                                                 |
| `publishedAt`     | datetime                                                                                     | optional                                                   | byline date / kicker                                                  |
| `coverImage`      | image (hotspot)                                                                              | **target:** ✅ required (post Ask 8) — _current:_ optional | hero context strip / event variant artefact (16:9 — see policy below) |
...
The **5 blocking schema changes** ship together — schema diffs land in `packages/sanity-schemas/src/article.ts`; data-migration / audit scripts land under `apps/studio/migrations/<name>/index.ts`. Per-blocker handling:

| #   | Blocker                                                          | Schema change                                                                                              | Data migration / audit                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --- | ---------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `article.lead` field added                                       | New optional `string` (~280 char) with Dutch description.                                                  | **None.** Additive optional field; existing articles default to `undefined`. Renderer falls back to `firstParagraphOf(body, 280)` when missing.                                                                                                                                                                                                                                                                                                                                                                                                     |
| 2   | `eventFact` required when `articleType=event`                    | Custom validator on `body` field.                                                                          | **Audit only.** Validator is render-time-enforced; existing articles in violation surface as Studio errors that editors fix at next edit. **Pre-deploy audit:** one-shot Studio query script lists `articleType==='event' && !body.some(b => b._type==='eventFact')` — editors backfill before validator deploys.                                                                                                                                                                                                                                   |
| 3   | `transferFact` required when `articleType=transfer`              | Same validator pattern.                                                                                    | Same audit script as #2 with predicate `articleType==='transfer' && !body.some(b => b._type==='transferFact')`.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 4   | `article.coverImage` becomes required (Ask 8)                    | `validation: (r) => r.required()`. Description updated to 16:9 contract.                                   | **Pre-deploy backfill required.** One-shot audit script lists every article where `coverImage` is null/undefined; output a slug + title list to the editorial team. Editorial team uploads a coverImage on each before the schema validator deploys. Anything left at deploy time blocks re-publish until fixed. (Per Ask 8 decision (a) — see "Migration plan" 4-step block above.)                                                                                                                                                                |
| 5   | `article.title` → constrained PT with `accent` decorator (Ask 9) | `type` swaps from `string` to single-block PT array; one custom `accent` decorator; no other marks/styles. | **One-shot data migration required.** Migration script transforms every existing article: `{ title: "Foo bar" }` → `{ title: [{ _type: "block", _key: ..., style: "normal", markDefs: [], children: [{ _type: "span", _key: ..., text: "Foo bar", marks: [] }] }] }`. **Consumer updates:** every `article.title` reader (cards, hero, OG meta, JSON-LD, sitemap, RSS, search index) switches to PT-aware rendering or uses a `serializeTitle(title): string` helper for non-display contexts. List the consumer audit in the 3.B.2 PR description. |

**Run order** (per `feedback_sanity_migrations`):

1. **Staging:** apply schema diffs + run all audit/migration scripts. Editorial team backfills any audit failures from #2, #3, #4.
2. **Production audit (read-only):** re-run #2/#3/#4 audit scripts against production dataset before production schema deploy. Confirm zero outstanding offenders.
3. **Production deploy:** schema diffs + #5 one-shot data migration. #1 is no-op data-wise. #4 validator activates.
4. **Post-deploy verification:** spot-check 5 random articles per `articleType` to confirm PT title rendering and coverImage required-state both work as expected.

The 5 blockers are intentionally bundled into a single migration — splitting them would create intermediate states where the schema and the renderer are out of sync (e.g. validator on but coverImage backfill incomplete).
