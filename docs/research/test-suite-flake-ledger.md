...
- Member: row 5. **Fixed** ([#3137](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3137)). It
  ...
  `browserContext.route` in `prepare` (`apps/web/.storybook/test-runner.ts`) that allows only the
  Storybook origin plus `data:`/`blob:` and aborts + logs everything else, replacing the old
  hand-maintained seven-host video deny-list; the Typekit face prefetched once per Storybook build
  into a gitignored local cache (`apps/web/scripts/prefetch-typekit.mjs`, served via `staticDirs`)
  since its terms forbid self-hosting; IBM Plex Mono self-hosted permanently under its OFL licence
  (`apps/web/public/fonts/ibm-plex-mono/`); and the vendored `waitForPageReady` replaced by a
  capped `waitForPageReadyCapped`. The same route removes class D row 26's live font CDNs too
