...
| 19  | `SearchInterface.test.tsx` (StrictMode timing) fails under sibling agents' `check-all`, passes alone                                                                                                                  | Local wave      | 2026-09-21                                                                                                                                           | C — CPU contention against a `waitFor` budget                                        | **fixed** by [#3143](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3143) — never reproduced, closed by rule: the file runs on fake timers, so the 350 ms debounce is simulated time and the four raised `waitFor` budgets are deleted | Reported independently by two wave agents. The file does use `StrictMode` + four `waitFor` blocks, which is the shape #2362 described                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- **Closed by a written budget, not a lint rule** ([#3143](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3143)):
  a test may not use more than half its own timeout, checked on demand from the slow-test lines
  every CI log already carries. A breach opens an issue, never a red check. No lint rule, because
  the cost is the iteration count, which no selector can see. The rule and the census command
  live in `apps/web/CLAUDE.md`.
