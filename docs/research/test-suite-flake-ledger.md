...
| Classes with at least one live member           | **7** (B, C, G, I, J, K, L)      |
| Classes fully closed by a named fix             | 4 (D, E, F, H); A mitigated only |
...
| 5   | `EditorialHero › Transfer Extension › smoke-test` — jest's 120 s per-test cap blown in the VR runner                                                                                                                  | VR              | 2026-09-21 ([run 35666231670](https://github.com/soniCaH/www.kcvvelewijt.be/actions/runs/35666231670))                                               | G — VR runner stall                                                                  | **fixed** ([#3137](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3137))                                                                                                                                                    | Turned `main` red; attempt 2 on the same commit passed 3041/3042 snapshots. Root-caused by [#3108](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3108): two live font CDNs, re-fetched per story by the a11y addon's cross-origin preloader, could black-hole a whole story file on the vendored page-ready wait. Closed by a deny-by-default `browserContext.route` in `prepare` (`test-runner.ts`), origin-matched against `TARGET_URL`, that fulfils the one Typekit host from a cache prefetched once per VR run and fails the story on any other denied request; IBM Plex Mono self-hosted under its OFL licence outside `public/`; and the vendored `waitForPageReady` cut down to this repo's own capped `waitForPageReadyCapped`                                                                                                                       |
...
| 12  | `next build` fails prerendering a route — `Sanity fetch failed` — three times: `TypeError: fetch failed` on a PR (2026-09-08), the same on `main` (2026-09-09), `HTTP 503 Service Unavailable` on `main` (2026-09-18) | CI build        | 2026-09-08 ([run 34212702776](https://github.com/soniCaH/www.kcvvelewijt.be/actions/runs/34212702776))                                               | H — build-time live network                                                          | **fixed** by [#3135](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3135) — see class H. Before it the class had no root-cause issue: the auto-filed #2883 and #3017 recorded only _that_ `main` was red, and never the why | Attempt 2 on the same commit built clean ([34212702776](https://github.com/soniCaH/www.kcvvelewijt.be/actions/runs/34212702776)). #2982's own verification pass reached the same verdict for [35318911017](https://github.com/soniCaH/www.kcvvelewijt.be/actions/runs/35318911017): "not a test flake… an upstream 503"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| 19  | `SearchInterface.test.tsx` (StrictMode timing) fails under sibling agents' `check-all`, passes alone                                                                                                                  | Local wave      | 2026-09-21                                                                                                                                           | C — CPU contention against a `waitFor` budget                                        | **fixed** by [#3143](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3143) — the file runs on fake timers, so the 350 ms debounce is simulated time and the four raised `waitFor` budgets are deleted                        | Reported independently by two wave agents. The file does use `StrictMode` + four `waitFor` blocks, which is the shape #2362 described                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 20  | `vr -u` accepts sub-threshold diffs and leaves stale baselines; parallel captures add sub-pixel noise to unrelated baselines                                                                                          | Local wave / VR | 2026-09-21                                                                                                                                           | K — shared CPU during capture                                                        | Stale-baseline half **fixed** ([#3136](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3136)); noise half **live**, unfiled                                                                                                  | Wave evidence, map Notes. Fix rule: `updatePassedSnapshot: true` in `.storybook/test-runner.ts`, proved by `pnpm --filter @kcvv/web run vr:accept:sub-threshold`. The noise half is class K with row 18; `-u` now writes that noise into the PNG instead of hiding it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| 26  | `TeamAgendaRow` — all 16 non-placeholder stories diff on **`--mobile` only**; a long team name truncates one character later (`Schoonbeek-Be…` → `Schoonbeek-Bev…`)                                                   | VR              | 2026-09-23 ([run 35827749813 attempt 1](https://github.com/soniCaH/www.kcvvelewijt.be/actions/runs/35827749813/attempts/1), job `Visual Regression`) | **D — fonts race the screenshot**, a class both of whose members were marked _fixed_ | **live**, unfiled — but see [#3137](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3137)                                                                                                                                    | Commit `4aedfc48` touched **comments only**, and attempt 2 passed with no code or baseline change. A shift of a whole character means the **measured text width changed** — antialiasing noise cannot do that. At the time, `apps/web/.storybook/preview-head.html` fetched `use.typekit.net` and `fonts.googleapis.com` **live** on every capture, and `apps/web/.storybook/test-runner.ts`'s `browserContext.route` aborted **video hosts only**, so neither font CDN was blocked. Rows 6–7 closed D with _waits_; `waitForFontsSettled` is capped per stage, and a capped wait expires. #3137 (class G) replaced both: a deny-by-default route plus a locally-served Typekit prefetch and self-hosted IBM Plex Mono, so neither font CDN is reachable during a capture any more. Re-verifying that this specific drift can't recur is its own check, not done here |
| 27  | `Input`/`Textarea`/`Select` "Focused" stories (`autoFocus`) diff on **`--mobile` only** — baseline shows the real focus ring, capture doesn't                                                                         | VR              | 2026-09-25 (full local `vr:check` run, [#3137](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3137))                                           | E — focus state is an environment property                                           | **fixed** ([#3137](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3137))                                                                                                                                                    | `waitForPageReadyCapped` (#3137) dropped a redundant `load`/`fonts.ready` wait that used to cost the first (mobile) viewport shot extra settle time while a live, uncached font fetch resolved — removing it exposed the pre-existing #3033 race sooner, not created it: `document.activeElement` is set by React's `autoFocus` immediately, but Chromium only paints `:focus` when the frame itself holds real OS focus, an environment property of whichever worker renders a given story. Fixed the same way as row 8: `fieldChrome.ts` gained a `data-[vr-force-ring=true]:` variant mirroring every `focus:` rule, and each affected story's `play` sets `data-vr-force-ring` via the shared `forceFieldFocusRing` helper (`_internal/field-vr-focus.ts`) instead of relying on the runner's real frame focus                                                    |
...
- Members: rows 11 (fixed, and closed by a lint rule — the model for how a class should end), 19 (fixed by #3143 — fake timers, so no wall-clock budget is left to race).
...
- The fix has shipped for class G: [#3137](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3137)'s **deny-by-default**
  `browserContext.route`, plus serving both stylesheets from disk instead of `use.typekit.net` /
  `fonts.googleapis.com` live. G and D shared one root, so this class's live font dependency is
  also gone — row 26 is left **live** above pending its own re-check (out of #3137's scope), not
  because the mechanism is unaddressed.
...
- Members: rows 8, 27. Both **fixed**, same mechanism (a `data-vr-force-ring` attribute a story's
  `play` function sets, bypassing the pseudo-class the runner's real frame focus can't be trusted
  to paint) — row 8 on `SearchForm`'s `focus-within:`, row 27 on the shared `fieldChrome.ts`
  `focus:` rules `Input`/`Textarea`/`Select` all use.
...
- Member: row 5. **Fixed** ([#3137](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3137)). It
  fired 2026-09-21, auto-opened `main is red` (#3094), and that issue auto-closed on the next green
  — so no ticket carried the cause until [#3108](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3108)
  root-caused it: not memory, but two live third-party font CDNs (`use.typekit.net`,
  `fonts.googleapis.com`) that `@storybook/addon-a11y`'s cross-origin stylesheet preloader
  re-fetches per story, and an unreachable host blocking the vendored `waitForPageReady`'s
  uncapped `load`-event wait. **The enforcement route that closes it:** a deny-by-default
  `browserContext.route` in `prepare` (`apps/web/.storybook/test-runner.ts`), origin-matched
  against `TARGET_URL` (not a bare "any localhost port" check), allowing only the Storybook origin
  plus `data:`/`blob:`, fulfilling the one Typekit host from a prefetched cache, aborting a small
  explicit list of known video-embed hosts, and recording + failing the story on anything else —
  replacing the old hand-maintained seven-host video deny-list; the Typekit face prefetched once
  per VR run (`apps/web/scripts/prefetch-typekit.mjs`, run from `vr:run`/`scripts/vr-docker.mjs`,
  read directly off disk via `route.fulfill` — never baked into the `storybook-static` CI uploads
  as a public artifact) since its terms forbid self-hosting, with `preview-head.html` keeping a
  live `<link>` so plain `pnpm storybook` dev still works unmodified; IBM Plex Mono self-hosted
  permanently under its OFL licence (`apps/web/.storybook/fonts/ibm-plex-mono/`, kept out of
  `public/` so it never ships to production as a dead asset); and the vendored `waitForPageReady`
  cut down to a capped `waitForPageReadyCapped`. The same route removes class D row 26's live font CDNs too
  (#3108 named the shared root), though row 26 itself is left as recorded here — verifying it
  needs its own re-check, out of scope for this ticket.
...
**Closed by #3135 (2026-09-23), with an enforcement route.** The deaths were never the
`generateStaticParams` enumeration: the logs name the **page render** of an enumerated slug
(`/staf/259` on 2026-09-09, `/evenementen/ploegvoorstelling-kampioenenviering` on 2026-09-18).
The rule, enforced by `apps/web/src/app/__tests__/isr-route-config.test.ts`:
...
A test body that uses most of its own timeout on an idle machine, so one slow runner turns it red
without any wave. Opened by [the shared-state sweep (#3124)](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3124).

- Member: `player-figure-variant.test.ts`, 83 % of the 5 000 ms default, solo. **Fixed** by
  [#3128](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3128) — assert once, not once per seed.
- **Closed by a written budget, not a lint rule** ([#3143](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3143)):
  a test may not use more than half its own timeout. The rule, the reason there is no lint rule,
  and the census command live in `apps/web/CLAUDE.md`.
- **Census, 2026-09-24:** 28 `ci.yml` runs, 13 of which ran the Vitest suites — **zero breaches**.
  The only test over 2 500 ms was `pre-commit.test.ts` at 4 299 ms of its own 60 000 ms timeout.
  The slowest default-timeout test was `trigger-psd-sync.test.ts` at 2 132 ms of 5 000 ms.
