...
> **Amended 2026-09-23 by [the Turbo cache miss](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3125).** The mechanism above is now measured, not
> inferred — on run [35785423276](https://github.com/soniCaH/www.kcvvelewijt.be/actions/runs/35785423276)
> Vercel reported success **6 min 15 s before** CI's build step replayed in 2 s, while `test` on the same
> tree missed and ran 4 m 53 s. `apps/web/vercel.json` sets `buildCommand` to
> `pnpm turbo build --filter=@kcvv/web`, so Vercel runs *this repo's Turbo task* into the same
> `sonicahs-projects` team cache.
>
> **A second cause this section does not have, which is why the asymmetry inverts on `main`.**
> `ci.yml:84` sets `environment: Preview | Production`, feeding the four `env` values `@kcvv/web#build`
> declares — so the build task has **two cache namespaces**. Same source, different hash: a PR and its
> `main` push share `test` hash `9c4ac63e05f00f2d` but carry `build` hashes `f675de47…` and `818ae110…`.
> Reproduced locally by toggling `NEXT_PUBLIC_SANITY_DATASET` alone. Vercel's *preview* build fills the
> PR namespace and wins by minutes; its *production* build fills the `main` one and **loses by 26–33 s**.
> Re-swept after `a1957099` over 42 runs: PRs **8 % `test` / 83 % `build`**, pushes **61 % / 39 %**.
> **Accepted, not fixed** — winning that race means CI waiting on Vercel.
...
- **`envMode` defaults to `strict`** ([reference/configuration#envmode](https://turborepo.dev/docs/reference/configuration#envmode)). The `build` task declares `env: [KCVV_API_URL, NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET, SANITY_API_READ_TOKEN]`; `test`, `lint` and `type-check` declare none. Turborepo's warning: "If you haven't defined the `env` or `globalEnv` keys for your task, Turborepo will not be able to use them when creating hashes. This means your task can **hit cache despite being in a different environment**." That is a wrong-hit hazard, not a missed-hit one. **Amended 2026-09-23 by [the Turbo cache miss](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3125): this hazard is **not live**.** A GitHub `environment:` injects nothing into the shell — each step must map its values, and only `Build Next.js` (`ci.yml:178-183`) and `e2e.yml:98-111` do; `ci.yml`'s workflow-level `env:` carries `TURBO_TOKEN` and `TURBO_TEAM` only. So during `Run tests` all four variables are **unset in both environments**, and the cross-environment replay cannot happen. It stays a **tripwire**: it goes live the day one of those four is mapped into a non-build step.
