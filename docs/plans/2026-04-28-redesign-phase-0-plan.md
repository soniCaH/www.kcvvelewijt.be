...
Run: `pnpm turbo run lint type-check test build --filter=@kcvv/web`
Expected: PASS.
...
>
  Drie afdelingen, 320 kinderen, één idee: plezier eerst. Onze academie draait
  op vrijwilligers, koffie en ouders die nog lang na de match blijven hangen.
</div>
...
<div
  style={{
    height: "120px",
    background: "var(--pattern-jersey-stripes)",
    border: "1px solid var(--color-paper-edge)",
  }}
/>
...
<div
  style={{
    height: "120px",
    background: "var(--pattern-jersey-stripes-tight)",
    border: "1px solid var(--color-paper-edge)",
  }}
/>
...
export function TicketStub({
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden="true"
        className="absolute top-1/2 -left-1 -translate-y-1/2 w-2 h-2 rounded-full bg-cream"
      />
      <span
        aria-hidden="true"
        className="absolute top-1/2 -right-1 -translate-y-1/2 w-2 h-2 rounded-full bg-cream"
      />
...
export type MonoLabelVariant =
  "plain" | "pill-jersey" | "pill-ink" | "pill-cream";
...
If any Stenciletta-using component is _visually critical_ (e.g. used in nav header or hero), flag it as a Phase 0 follow-up: that component's redesign moves up the schedule, OR Stenciletta is reinstated until the component is rebuilt.
...
If `vr:update` reports any _update_ to an existing baseline (rather than a new capture), STOP. Inspect the diff PNG with `pnpm --filter @kcvv/web run vr:diff <story-id>` and confirm the change is intentional. If unintentional, fix the offending change before continuing.
...
**Step 1: Run the full turbo gate suite**
...
- [x] `pnpm turbo run lint type-check test build --filter=@kcvv/web` green.
...
_End of Phase 0 implementation plan._
