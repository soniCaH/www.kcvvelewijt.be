...
| Checkpoint D — SiteFooter                        | ✅ **locked (2026-05-05)** — synthesis of A/C/D ingredients (B eliminated). One `footer-locked.md` in `docs/design/mockups/phase-3-d-footer/`. **Picks:** Q1=H3 (modest wordmark + 2-colour motto, cream paper) · Q2=I2 (task-oriented columns: Ontdek · Aansluiten · Bij de club) · Q2.5=J1-refined (role-based items: Als speler · Als vrijwilliger · Als sponsor; cashless dropped) · Q3 verifications (sponsor strip stays separate above footer · `<PosterWordmark>` absorbed by H3 · every-page render · 3-col → 1-col mobile collapse). Owner-direction scope corrections: founding year `1909` (corrects MissionBanner's 1948, PageHeader wordmark's 1948, jsonld.ts's 1924 — fix in passing during 3.C.3 implementation) · no newsletter. New shared sub-component: `<FooterLink>` — Storybook coverage required per reuse mandate. **No schema migrations required for Checkpoint D.** |
