...
8. **Icon library — Lucide vs a retro replacement?** Lucide's geometric crisp 2px-stroke aesthetic is unapologetically modern and clashes with the printed-fanzine direction. Saved-memory preference (`feedback_visual_preferences.md`) already favours **typographic glyphs over Lucide where the glyph reads** — `→`, `←`, `★`, `✦`, `✓`, `↓`, `☞`, `✏︎` typed directly in body/mono font cover ~60% of icon use cases. For the long tail, candidates surveyed: **Phosphor** (`fill` weight reads as ink-stamp solid; cheapest swap-for-Lucide path), **Game Icons** (game-icons.net, CC BY 3.0, hand-drawn engraving feel with ~4000 SVGs including football/whistle/scarf/trophy — strongest free option), **Streamline Hand-Drawn** (paid, pixel-perfect for editorial-magazine), **custom-commissioned set** (best brand fidelity, ~30 icons, ~3-6 weeks). **Recommended hybrid:** Tier 1 typographic glyphs first; Tier 2 custom hand-drawn SVGs for the ~15-25 unique icons that carry brand weight; Tier 3 a retro icon library for the long tail. Phase 0 ships zero icons; the actual decision lands at the **Phase 2 atom-rework design checkpoint** when icon-using atoms get touched. A side-by-side comparison artefact lives at `docs/design/exploratory/phase-2-icons.html` for visual A/B at decision time. (from owner question 2026-04-28.)
