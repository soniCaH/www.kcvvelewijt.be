...
The redesign is using Storybook + `@storybook/test-runner` for visual regression coverage. Phase 1–3 of the VR rollout (commits `feat(ui): vr phase 1..3`, merged April 2026) tagged ~167 stories with `vr` and committed 978 baselines. The Phase 4 issue (#1375 — "real-page composition coverage") was scoped to extend that coverage to full `Pages/*` compositions; Phase 5 (#1376 — "full Storybook coverage via fixture pinning") was scoped to fixture-pin everything that wasn't yet stable.
...
1. **Drop `Pages/*` from Storybook VR coverage.** The five flaky stories tagged `vr-skip` in the same PR as this PRD; the rest of the `Pages/*` suite is excluded by tag at the test-runner level (`--excludeTags vr-skip` already exists). New `Pages/*` stories added during the redesign do NOT get `vr` baselines.
