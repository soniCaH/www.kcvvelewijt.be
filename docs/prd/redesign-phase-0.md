...
| `<HighlighterStroke>`                                  | `variant: 'a' \| 'b' \| 'c'` (three hand-drawn SVGs), `color: 'jersey'`                                                              | Wraps children in a single `<span>` and applies the SVG stroke as a CSS `background-image` (data URL). **Phase 0 ships single-line only**; multi-line wrapping support is deferred to Phase 1+ — the story file documents this with a `MultiLineUnsupported` story flagged `parameters: { vr: { disable: true } }`. SVG paths live in `apps/web/src/components/design-system/HighlighterStroke/strokes.ts` as a const map keyed by variant. |
