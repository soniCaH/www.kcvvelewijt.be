...
| 5.d2 | ~~Subject avatar vocabulary~~ **LOCKED 2026-05-18 → D (scale-conditional).**                                                         | ~~(A) photo / (B) monogram / (C) illustration / (D) mixed~~ — Q&A row scale (~32px) renders **monograms** (first initial of `firstName` on a `bg-jersey-deep` disc, cream full opacity). Pull-quote attribution scale (~64px) renders **circular photo crops** with R9 newsprint treatment, falling back to a 64px monogram when no photo exists. Mockups: `5d2-avatars/`. Lock: `avatar-locked.md`.                                                                    | Subject-presence vocabulary across `<QASection>` + `<PullQuote>` attribution. |
