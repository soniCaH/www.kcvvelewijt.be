...
| `<MatchRecapStats>`     | 5.B.mat | **Dropped — superseded by the 5.d-mat lock (2026-06-05).** No standalone stats/lineup block was built: lineups stay canonical on `/wedstrijd/[matchId]` and `MatchDetail` carries no statistics (mocking = data-fabrication, forbidden). The surviving recap body content shipped in #1470 as `<MatchGoalsBlock>` (goalscorer roll-call) + the hero two-tier score bar. See `docs/design/mockups/phase-5-article-detail/match-locked.md`. |
...
| 5.B.mat | #1799 | MatchRecapStats (dropped — superseded by 5.d-mat lock)            | #1791, #1793, #1470               | CLOSED 2026-06-19 (not planned; recap body shipped via `<MatchGoalsBlock>` + hero score bar in #1470)                                                |
