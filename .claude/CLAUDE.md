...
[^api-contract]: `lint` (`eslint .`, `typescript-eslint` mirrored from `@kcvv/api`) and `type-check` (`tsgo --noEmit`) run in CI's "Lint + type check Studio" step ([#3119](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3119)); `build` (`tsgo --build`) runs too — transitively via `^build` when `@kcvv/web`/`@kcvv/api` type-check or build, and directly in the deploy and E2E workflows (`ci.yml` "Build api-contract" steps, `e2e.yml`).
