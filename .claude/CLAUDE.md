...
[^vr]: Storybook VR runs pixel diffs at 3 viewports via `test-storybook`, scoped to `vr`-tagged stories only (`--includeTags vr --excludeTags vr-skip`, `apps/web/package.json`'s `vr:run`) — 183 of 208 story files carry the `vr` tag. The 21 `Pages/*` stories carry none and are never screenshotted (consistent with `apps/web/CLAUDE.md`'s "not VR-tested" note) — but as of [#3188](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3188) they, and every other non-`vr` story, ARE visited for accessibility (`vr:axe:non-vr`, `--excludeTags vr`; see the `[^a11y]` note). Runtime geometry (`play`) moved to Storybook's own `play` exports, run by `@storybook/addon-vitest` (`pnpm test:storybook`, `apps/web/vitest.storybook.config.ts`) — [#3146](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3146) is done; `scroll-arrows.spec.ts`/`section-nav.spec.ts` no longer exist.

[^a11y]: **Authoritative text — `apps/web/CLAUDE.md` points here rather than repeating these numbers, so the two copies cannot drift.** `@storybook/addon-a11y` (`.storybook/main.ts`) runs inside the same `vr`-tagged run described in the note above, plus a dedicated axe-only step in the same job scoped to every story that carries no `vr` tag at all (`apps/web/package.json`'s `vr:axe:non-vr`, `--excludeTags vr` — no screenshot, no new baselines; not a hand-added opt-in tag, so a new non-`vr` story is covered automatically) — together every `vr`-tagged story plus every `Pages/*`/other non-`vr` story. Two stories (`StandingsTable`'s `StickyColumnsPinned`, `OrganigramExplorer`'s `ZoomOverflowsTheStage`) also carry `vitest-play-only`: their `play()` needs `@storybook/addon-vitest`'s own `setViewport()`, which `test-storybook` has no equivalent for, so they stay on the `pnpm test:storybook` layer exclusively. **Decided by the owner on 2026-09-25: Storybook VR owns accessibility, and gates as of [#3188](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3188).** `parameters.a11y.test = "error"` (`.storybook/preview.ts`) fails the job on any violation. `color-contrast` is disabled globally, in exactly one place (that same `parameters.a11y.config.rules` entry) — the club has no WCAG conformance target; legibility is judged by eye, ties broken by contrast at most (#2395) — every other rule gates. The 2026-09-25 pre-fix census measured ~70 non-contrast violations over 13 components (`label`/`label-title-only`/`select-name` on the form atoms, `aria-allowed-role` and `heading-order` in `ArticleBody`/`QASectionDivider`, `aria-required-children` in `HubSearch`/`OrganigramExplorer`, `svg-img-alt` in the calendar subscribe QR code, `listitem` in `TapedCardGrid`, and `landmark-no-duplicate-banner`/`landmark-unique` in `ContactPage`); a fresh census with contrast off now shows 0, including on `MapEmbed`/`SkeletonBars`/`SectionNavChip`/`LoadingAnnouncement` — four components that carried neither `vr` nor any axe coverage before the non-`vr` sweep replaced the earlier hand-tagged approach.
