...
3. **Quality before commit:** `pnpm --filter @kcvv/web lint:fix` then `pnpm turbo run lint type-check test build --filter=@kcvv/web` — swap `@kcvv/web` for the workspace you changed; a shared package gets a leading `...` (`--filter=...@kcvv/api-contract`) so its dependents are checked too
