...
[^vr]: `test-storybook` (`apps/web/package.json`'s `vr:run`) visits every story now, sharded 3 ways — no `--includeTags`/`--excludeTags` filter decides which stories get VISITED any more (#3188, review round 2). `.storybook/test-runner.ts`'s `postVisit` decides per story whether to screenshot: only when the story's combined tags include `vr` and exclude `vr-skip` (183 of 208 story files carry `vr`). Every other story — the 21 `Pages/*` files (carry no `vr` tag, consistent with `apps/web/CLAUDE.md`'s "not VR-tested" note) and any other component whose story was never tagged `vr` — still gets its accessibility check (see the `[^a11y]` note), just no screenshot and no baseline. Runtime geometry (`play`) moved to Storybook's own `play` exports, run by `@storybook/addon-vitest` (`pnpm test:storybook`, `apps/web/vitest.storybook.config.ts`) — [#3146](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3146) is done; `scroll-arrows.spec.ts`/`section-nav.spec.ts` no longer exist.

[^a11y]: **Authoritative text — `apps/web/CLAUDE.md` points here rather than repeating these numbers, so the two copies cannot drift.** `@storybook/addon-a11y` (`.storybook/main.ts`) runs on EVERY story `test-storybook` visits (see the `[^vr]` note) — one run, no separate axe-only invocation: whether `postVisit` also takes a screenshot for a given story is orthogonal to whether the a11y check ran, since that check comes from Storybook's own render lifecycle before `postVisit` gets a say. `.storybook/test-runner.ts`'s `preVisit` also honours a story's `globals.viewport.value` (resolved against `.storybook/preview.ts`'s viewport options, `page.setViewportSize` before mount) — the same mechanism `@storybook/addon-vitest` uses for `pnpm test:storybook` — so a `play()` fixture that depends on a named viewport runs correctly under either runner, no runner-specific opt-out tag needed. **Decided by the owner on 2026-09-25: Storybook VR owns accessibility, and gates as of [#3188](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3188).** `parameters.a11y.test = "error"` (`.storybook/preview.ts`) fails the job on any violation. `color-contrast` is disabled globally, in exactly one place (that same `parameters.a11y.config.rules` entry) — the club has no WCAG conformance target; legibility is judged by eye, ties broken by contrast at most (#2395) — every other rule gates. The 2026-09-25 pre-fix census measured ~70 non-contrast violations over 13 components (`label`/`label-title-only`/`select-name` on the form atoms, `aria-allowed-role` and `heading-order` in `ArticleBody`/`QASectionDivider`, `aria-required-children` in `HubSearch`/`OrganigramExplorer`, `svg-img-alt` in the calendar subscribe QR code, `listitem` in `TapedCardGrid`, and `landmark-no-duplicate-banner`/`landmark-unique` in `ContactPage`); a fresh census with contrast off now shows 0, including on `MapEmbed`/`SkeletonBars`/`SectionNavChip`/`LoadingAnnouncement` and the 7 stories across 6 files that combine an inherited `vr` tag with a per-story `vr-skip` — none of which any single CLI-scoped invocation reached before this round.

[^studio]: Config only (`sanity.config.ts`, `structure.ts`, `sanity.cli.ts`) plus migrations. No `type-check` script exists on this package itself, but `turbo build --filter=@kcvv/studio` (`sanity build`) runs in CI, as does the "Sanity types in sync" gate (`sanity schema extract && sanity typegen generate`, diffed against the committed types). Two untested surfaces remain, neither closed by build/lint: 11 of 26 migrations still hold logic in place (435 lines, closes with [#3153](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3153)), and `apps/studio/scripts/` — 5 files, 981 lines (`migrate-drupal-node.ts`, `remap-qa-respondent-keys.ts`, `seed-e2e-fixtures.ts`, `seed-interview-qa-pairs.ts`, `seed-pull-quote-example.ts`) — which #3153 does **not** cover.
...
[^sanity-schemas]: 4 195 lines, 0 tests. `lint` (`eslint .`, `@sanity/eslint-config-studio` mirrored from `@kcvv/sanity-studio`) and `type-check` both run in CI's "Lint + type check Studio" step ([#3119](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3119)). Its `build` script (`tsgo --noEmit`, functionally identical to `type-check`) also runs transitively via `turbo.json`'s `^build` whenever `@kcvv/web` or `@kcvv/sanity-studio` type-checks or builds, and the "Sanity types in sync" gate's `sanity schema extract` parses every schema here too. `validation/`, `preview/` and `blocks/` (629 lines of pure logic) are Vitest's named gap ([#3100](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3100) decision D9) — decided, not shipped, no open ticket. Schema declarations stay untested by design.

[^api-contract]: `lint` (`eslint .`, `typescript-eslint` mirrored from `@kcvv/api`) runs in CI's "Lint API" step ([#3119](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3119)). `type-check` (`tsgo --noEmit`) is never invoked — deliberately, since `build` (`tsgo --build`) checks the same code — transitively via `^build` when `@kcvv/web`/`@kcvv/api` type-check or build, and directly in the deploy and E2E workflows (`ci.yml` "Build api-contract" steps, `e2e.yml`).
...
3. **Quality before commit:** `pnpm --filter @kcvv/web lint:fix` (web only — no other workspace has `lint:fix`) then `pnpm turbo run lint type-check test build --filter=@kcvv/web`. Changed another workspace? Filter on it instead; a shared package gets a leading `...` so its dependents are checked too: `--filter=...@kcvv/api-contract`, and `--filter=...@kcvv/sanity-schemas` for either Sanity package (that one also reaches studio typegen and the web build)
