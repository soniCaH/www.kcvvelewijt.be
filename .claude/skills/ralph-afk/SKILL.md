...
| any other `apps/*/src` or `packages/*/src` | the same command filtered on that workspace, with a leading `...` so every dependent is checked too (`--filter=...@kcvv/api-contract`; `--filter=...@kcvv/sanity-schemas` for either Sanity package) — the web gate proves nothing about `apps/api` |
