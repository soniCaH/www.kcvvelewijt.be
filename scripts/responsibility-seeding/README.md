...
migration, the reason this package's two follow-up scripts exist, not
...
not justify deleting the package: `scripts/board-cleanup`, `scripts/staff-cleanup`
...
this package's `sanity-client.ts` (both the safe-by-default `client` and the
...
default. See `src/draft-id.ts` and its use in
