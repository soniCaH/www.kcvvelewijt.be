...
export type PullQuote = {
  ...
  speaker?: PlayerReference | StaffMemberReference;
  externalName?: string;
  externalRole?: string;
...
// Query: *[_type == "homePage"][0] {    "bannerSlotA": bannerSlotA-> {      "imageUrl": image.asset->url + "?w=1200&q=80&fm=webp&fit=max",      "imageDimensions": image.asset->metadata.dimensions{width, height},      alt,      href    },    "bannerSlotB": bannerSlotB-> {      "imageUrl": image.asset->url + "?w=1200&q=80&fm=webp&fit=max",      "imageDimensions": image.asset->metadata.dimensions{width, height},      alt,      href    },    "bannerSlotC": bannerSlotC-> {      "imageUrl": image.asset->url + "?w=1200&q=80&fm=webp&fit=max",      "imageDimensions": image.asset->metadata.dimensions{width, height},      alt,      href    },    "matchesSliderPlaceholder": matchesSliderPlaceholder {      nextSeasonKickoff,      announcementText,      announcementHref,      "highlightImage": highlightImage {        alt,        "asset": asset->{          "url": url + "?w=1344&h=320&q=80&fm=webp&fit=crop&crop=focalpoint&fp-x=" + string(coalesce(^.hotspot.x, 0.5)) + "&fp-y=" + string(coalesce(^.hotspot.y, 0.5)),          "lqip": metadata.lqip        }      }    },    "youthPlayerCount": youthPlayerCount,    "youthTeamCount": youthTeamCount  }
export type HOMEPAGE_QUERY_RESULT = {
  ...
  youthPlayerCount: string | null;
  youthTeamCount: string | null;
} | null;
...
// Query: *[_type == "responsibility" && active == true] | order(title asc) {  "id": slug.current,  title,  "role": audience,  question,  keywords,  summary,  category,  icon,  "primaryContact": primaryContact {    contactType,    teamRole,    "position": organigramNode->title,    "roleCode": organigramNode->roleCode,    "members": organigramNode->members[]->{      "id": _id,      "name": coalesce(firstName, "") + " " + coalesce(lastName, ""),      email, phone    },    "nodeId": organigramNode->_id,    "role": role,    "email": email,    "phone": phone,    "department": department  },  "steps": steps[] {    description,    link,    "contact": select(defined(contact) => contact {      contactType,      teamRole,      "position": organigramNode->title,      "roleCode": organigramNode->roleCode,      "members": organigramNode->members[]->{        "id": _id,        "name": coalesce(firstName, "") + " " + coalesce(lastName, ""),        email, phone      },      "nodeId": organigramNode->_id,      "role": role,      "email": email,      "phone": phone,      "department": department    }, null)  },  "relatedPaths": coalesce(relatedPaths[]->slug.current, [])}
...
// Query: *[_type == "team" && slug.current == $slug && archived != true][0] {  _id, psdId, name, displayName, "slug": slug.current, age, gender, footbelId, division, divisionFull,  tagline, body[]{ ..., "fileUrl": file.asset->url }, contactInfo,  "teamImageUrl": teamImage.asset->url + "?w=1200&h=800&q=80&fm=webp&fit=crop&crop=focalpoint&fp-x=" + string(coalesce(teamImage.hotspot.x, 0.5)) + "&fp-y=" + string(coalesce(teamImage.hotspot.y, 0.5)),  players[]-> {    _id, psdId, firstName, lastName, jerseyNumber, keeper, positionPsd, position,    "psdImageUrl": psdImage.asset->url + "?w=400&q=80&fm=webp&fit=max",    "transparentImageUrl": transparentImage.asset->url + "?w=600&q=80&fm=webp&fit=max"  },  staff[] { role, "member": member-> {    _id, psdId, archived, firstName, lastName, functionTitle,    "photoUrl": photo.asset->url + "?w=300&h=400&q=80&fm=webp&fit=crop&crop=focalpoint&fp-x=" + string(coalesce(photo.hotspot.x, 0.5)) + "&fp-y=" + string(coalesce(photo.hotspot.y, 0.5)),    // fit=max, not the hotspot-crop route photoUrl above uses: psdImage has    // no hotspot option on the schema (it's readOnly — no editor could ever    // set one), so a hotspot-crop URL here would always centre-crop a    // 350x350 PSD square with no way to adjust it. Matches the player    // psdImageUrl projection above (#2895 review). A staff member who needs    // specific framing uses the editorial 'photo' field, which keeps its    // hotspot and still wins at render.    "psdImageUrl": psdImage.asset->url + "?w=400&q=80&fm=webp&fit=max",    "hasBio": count(bio) > 0  } }}
