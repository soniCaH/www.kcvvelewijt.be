import { describe, it, expect } from "vitest";
import { render, screen } from "@testing-library/react";
import { JeugdHero } from "./JeugdHero";

describe("JeugdHero", () => {
  it("renders label, title with green accent, and subtitle", () => {
    render(<JeugdHero />);

    // Label
    expect(screen.getByText("Jeugdopleiding")).toBeInTheDocument();

    // Title parts
    expect(screen.getByText(/de toekomst/i)).toBeInTheDocument();
    const greenAccent = screen.getByText(/elewijt/i);
    expect(greenAccent).toBeInTheDocument();
    expect(greenAccent.tagName).toBe("SPAN");
    expect(greenAccent.className).toContain("text-kcvv-green");
  });

  it("renders subtitle about 200+ players", () => {
    render(<JeugdHero />);
    expect(screen.getByText(/200 jonge voetballers/i)).toBeInTheDocument();
  });

  it("renders built-in diagonal SVG", () => {
    const { container } = render(<JeugdHero />);

    const svg = container.querySelector("svg");
    expect(svg).toBeInTheDocument();
    expect(svg).toHaveAttribute("viewBox", "0 0 100 100");
    expect(svg).toHaveAttribute("preserveAspectRatio", "none");

    // Lower-left triangle filled with gray-100
    const polygons = container.querySelectorAll("polygon");
    expect(polygons.length).toBeGreaterThanOrEqual(2);

    const grayPolygon = Array.from(polygons).find(
      (p) => p.getAttribute("fill") === "var(--color-gray-100)",
    );
    expect(grayPolygon).toBeInTheDocument();
  });
});
