...
`@storybook/addon-a11y` (registered in `.storybook/main.ts`) **does run** — inside the visual-regression job, on every `vr`-tagged story, plus every story that carries no `vr` tag at all (`vr:axe:non-vr`, `--excludeTags vr`) via a dedicated axe-only step that never screenshots — so `Pages/*`, and any other component whose story was never given the `vr` tag, is axe-checked without becoming VR-tested (the line above is still accurate: no baseline exists or is added for it). A handful of stories carry `!vr` (removing an inherited `vr` tag) specifically because their `play()` depends on `@storybook/addon-vitest`'s own `setViewport()` (a `globals.viewport` value `test-storybook` has no equivalent for) — the two whose assertions are viewport-sensitive enough to actually fail under `test-storybook`'s own default viewport also carry `vitest-play-only`, which the axe-only step excludes too; that layer already covers them (`pnpm test:storybook`, next section). **Storybook VR owns accessibility** (decided 2026-09-25) **and gates as of #3188** — see root `.claude/CLAUDE.md`'s workspace table (`apps/web` row, accessibility note) for the measured violation count and the `color-contrast` exclusion. That note is authoritative; this line does not repeat its numbers.
