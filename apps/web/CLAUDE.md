...
- A decorator that sets `window.location.hash` during render (needed for a cold-load fixture's timing — see the hash-click bullet above) must reset it in its OWN unmount effect, not a `finally` in `play`: `play` never runs for an autodocs-page render, but the decorator still mounts there and must still clean up (`test/storybook/ColdLoadHashDecorator.tsx`, `@test-storybook/ColdLoadHashDecorator`). Tag a story built on it `["!autodocs"]` too — the docs page renders every story's decorator on one shared page, and this one has no business mutating the shared `location.hash` just because it is being displayed.
