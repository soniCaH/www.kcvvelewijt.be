...
- [ ] **GTM updated**: new event names not already matched by the `responsibility_|search_|organigram_|related_content_|homepage_|directions_|firstteam_strip_|article_|related_article_|event_cta_|article_video_|team_` regex need a new trigger/tag in GTM; new event parameters need a Data Layer Variable (DLV) created in GTM and mapped into the GA4 Event tag's parameter fields
