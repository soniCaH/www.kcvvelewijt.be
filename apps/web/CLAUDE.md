...
- **The local VR container is exclusive, machine-wide, and never builds its own image.** `vr-docker.mjs` fails fast, naming the build command, when `kcvv-vr-runner:latest` is absent (a wave builds it once, in step 0 — never inside a lane), and holds a `mkdir`-based lock so only one container runs at a time on this machine — four concurrent containers would want its entire memory. A refusal names the fix (build the image, or wait for the other run); it is never yours to bypass. Full register: `.claude/skills/ralph-afk/SKILL.md` (#3141).
