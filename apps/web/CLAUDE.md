...
- **Before adding a page-level runtime guard, trace the repository GROQ filter.** A page component gating on `article.publishedAt && …` is dead code if the repository's `findBySlug` already has `publishAt <= now()` in its GROQ. Duplicate guards mislead reviewers into flagging legitimate emission paths and hide the real filter if it ever changes. Read the repo method first; gate only at the layer that owns the concern.
