...
One known exception is still shipping: `ContactCard`'s icon-only action (`h-10 w-10`, 40px), `OrganigramExplorer`'s two sibling-navigation arrows (14px icon + padding + border, ~28px), `HubSearch`'s clear button (16px icon + pull-back padding, ~28px), `MatchStripView`'s result/fixture toggle arrows (`h-9 w-9`, 36px), and `CalendarWidget`'s period-navigation arrows (`h-8 w-8` from `md` up — 44px only below it) are all icon-only and under 44px. Not fixed here — the owner is deciding on a follow-up.
