...
**A control whose visible box carries its own border cannot use that idiom** — padding grows the border along with the hit area. It takes the `hit-area` utility (`apps/web/src/app/globals.css`) instead: the control becomes `position: relative` and a transparent `::before` at `inset: calc(50% - 22px)` supplies a 44 × 44 hit area around any box up to 44px, while the visible box stays exactly as drawn. The same containment rule applies — the overhang lands in the container's own gap or padding, no ancestor may clip it, and it never reaches a neighbour. `ContactCard`'s `E-mail` / `Bel` actions (40px), `OrganigramExplorer`'s sibling arrows (28px), `MatchStripView`'s result/fixture toggle (36px) and `HubSearch`'s clear button (32px hero, 28px nav — on top of its pull-back padding) use it; `HubSearch`'s text field gives back an end margin (4px hero, 6px nav) while the button shows, so the hit area never covers the field. `CalendarWidget`'s period arrows take a visible `h-11 w-11` at every width instead, because an invisible hit area there would poke out of the toolbar row. Decided on [#3076](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3076).
