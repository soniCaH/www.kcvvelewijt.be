...
**The Two-Line Hero Rule.** `leading-hero` (`--leading-hero: 0.85`) is a ramp step, not a `--text-*` step — it carries no font-size of its own, so it composes with whichever display step a two-line hero's own lines already use. Reserved for heroes that are a **fixed, structural** two-line composition — always exactly two lines regardless of content, never a single string that happens to wrap (`<PlayerHero>` / `<StaffHero>`'s first-name-over-last-name rhythm). **Never the default**, and never applied to a headline whose line count depends on content length or viewport (an `<EditorialHero>` title can render 1–3 lines depending on what an editor typed; there is no CSS way to detect the rendered line count, so a step this tight cannot govern it). Pair it with a small `padding-top` (an em value, scaled to that line's own font-size) on the tallest line, or its ascenders crowd whatever sits above it (D14/Y1, decision-sheet §8, #2617).

**The Hanging Mark Rule.** A pull quote's opening mark (`<QuoteMark>`) hangs outside the card's padding via `hanging-punctuation: first` where Safari supports it, and a negative `text-indent` fallback everywhere else — so the card's actual reading column (the blockquote body) keeps a straight left edge instead of the mark's own ink notching into it. The two mechanisms are gated by `@supports not (hanging-punctuation: first)` so they are structurally mutually exclusive: applying both would hang the mark twice (D14/Y6, decision-sheet §8, #2617).
