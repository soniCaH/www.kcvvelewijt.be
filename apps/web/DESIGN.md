...
- **Mono** — `text-mono-md` (500, 0.875rem, lh 1.4): scores, dates, stat values, inline data — a number that **annotates** something else (a fixture-row score, a minute beside a scorer, a shirt number beside a name, a standings cell, a day stub). A number that **is** the surface's own subject — the match-detail hero score, a `<NumberDisplay>` stat — keeps its display face instead (see **The Subject/Tag Rule** below); mono is the tag register, not every number's register.
