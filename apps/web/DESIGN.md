...
**The Hyphenation Rule.** A slot carries `hyphens-auto` when its narrowest real width cannot hold its longest **possible** word — not its longest word today. Free-text editorial content and model output are unbounded: a role title, an article headline and an AI answer have no longest word until someone writes one, so they can never be proven to fit, and they carry the declaration on that basis alone — the news headline (`NewsCard`) is the example. Hardcoded copy and fixed value sets are bounded: measure them, and if the measurement clears, they carry nothing — `UltrasHero`'s `De luidste hoek` is a literal in the component with a 7-character longest word, so it provably fits and stays undeclared. Where a slot hyphenates, `hyphens-auto` goes in **alone, never paired with `break-words`** — the per-character emergency breaker out-competes the hyphenator in the greedy line-breaker and produces hyphen-less mid-word clips ([#2269](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2269)). The measurement that forced the bounded/unbounded split rather than a fixed width list: `NewsGrid`'s narrowest real text column is ~141px (`sm:grid-cols-3` from a 640px viewport), where a live headline word renders 188px — the original drill measured only a 319px slot and generalised from it. Full table and rendered drill: [#2586](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2586#issuecomment-5756306215).
