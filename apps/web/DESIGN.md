...
**No Filler Photo, Ever Rule (#2965).** This is the render-time Writer Rule applied to imagery, for a subject kind that has no artefact of its own: a `variant="nav"` `<EditorialHubCard>` (the pinned nav tiles on `/jeugd`) shows a photo when its `NavCardConfig.image` has one, and keeps the flat `bg-jersey-deep` + 40px glyph, unchanged, when it does not. A generic stand-in photo is never substituted for a missing one — a photo on "Blessure of medisch attest?" would read worse than an honest icon, the same trap the Phase 4.5 typographic cards fell into ("they don't shine at all"). Present renders the photo at 16:9 behind a `jersey-deep-dark` scrim (~62%) with the pill and glyph on top; the photo is decorative (`alt=""`) because the card is already a labelled link. Assets live in `apps/web/public/images/jeugd/`, not Sanity — chrome, not editorial content, so an editor deleting an article can't break a nav tile.
