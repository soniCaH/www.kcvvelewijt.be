...
A slot may carry **both** `hyphens-auto` and `break-words` when its content is Dutch prose that can also contain an arbitrary token with no dictionary break point — a URL, an e-mail, a coined compound. Model output is the case this reaches today: `SearchAnswerCard` pairs both, hyphenating the prose by Dutch rules and falling through to an unbroken-anywhere break only where hyphenation has nothing to work with. [#2269](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2269) held that `break-words` suppresses the hyphen — that finding does not reproduce in Chrome against the real faces at real widths (checked on #2586: the pairing renders the hyphen, identical to `hyphens-auto` alone). **This is verified in Chrome only.** The four slots that currently carry `hyphens-auto` alone (`NewsCard`, `OrgPersonCard`, `EditorialHeroShell`, `MemberDetailPanel`) stay alone pending a Safari check — pairing them has no safety net if Safari does suppress the hyphen there, where `SearchAnswerCard` already shipped `break-words` alone before this rule existed, so the worst case of pairing it is the status quo. Keep the #2269 reference on any slot this touches; it is the reason a pairing needs verifying before it is added, not a settled prohibition.
