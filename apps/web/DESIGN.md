...
**The Type Ramp Freeze Rule.** The twelve steps above are the whole ramp — an arbitrary `text-[9px]`, `text-[10.5px]`, `text-[1.05rem]`, `text-[0.4em]` or `text-[length:var(--text-*)]` bypasses it outright, or bypasses it while looking token-driven (see "Reach for the class" above). `eslint.config.mjs`'s `no-restricted-syntax` block enforces this on every new `className` in `src/`: writing a fresh off-ramp literal is now a lint error, not a style note. It does not enforce it retroactively — the **314** literals that already existed across **87** files the day the rule landed are frozen, not fixed, via ESLint's native bulk-suppressions feature (`--suppress-all`), snapshotted in the committed `apps/web/eslint-suppressions.json`. That count is the drain metric: ordinary `pnpm lint` (CI included) passes today only because those exact call sites are suppressed, and it starts failing the moment anyone adds a new one. Draining a call site — replacing the literal with its token — makes the suppression for that file stale; running `pnpm --filter @kcvv/web lint:prune` (`eslint . --prune-suppressions`) removes it, so the file's count can only shrink, never grow, and there is no hand-rolled allowlist to fall out of sync. This ticket froze **font-size literals only**: `leading-*` line-height literals are a separate, not-yet-built rule (#2666) and are untouched here. Decided and built by [#2418](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2418).
