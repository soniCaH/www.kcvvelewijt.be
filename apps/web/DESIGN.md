...
- **Ink Soft** (`#1f1f1f`): a dark surface layered on a dark surface. A surface only — never a text colour (see The Two-Tier Text Rule).
- **Ink Muted** (`#6b6b6b`): the quiet voice — bylines, timestamps, metadata. The soft shadow offset and `border-ink-muted` borrow the value without claiming the role.
- **Cream Quiet** (`--color-cream-quiet`, cream at 70% alpha): the quiet voice on dark, `ink-muted`'s mirror. An alpha, not a hex, so it adapts to `jersey-deep-dark` and ink alike. Never on `jersey-deep` (see The Whole-Cream Rule).
...
**The Two-Tier Text Rule (#2551).** Text has two tiers, not three. On cream, `text-ink` is the voice and `text-ink-muted` is the quiet voice. On dark, `text-cream` is the voice and `text-cream-quiet` is the quiet voice. The split is by **role**, not by file: body voice — leads, prose, card deks, italic notes, a sentence of any kind — takes the loud tier; metadata — bylines, timestamps, mono micro-labels, figure captions — takes the quiet tier. There is no middle tier. `ink-soft` sits 1.20:1 from `ink`, so as a text colour it is not a tier at all, and a real middle value would be the second neutral grey the No-Grey-UI Rule forbids. `text-ink-soft` and every `text-cream/NN` are lint errors (`apps/web/eslint.config.mjs`, next to the Token-Only Colour Rule); `/85` in particular is the dark side's `ink-soft` and is rejected for the same reason. Controls (chips, links, buttons) take their tone from their own primitives, not from this rule.

**The Whole-Cream Rule.** Cream on a jersey-deep surface is cream, never a fraction of it. `text-cream/90` costs more contrast than a whole shade step of green does (4.05 → 3.56, where the #2395 shade step bought 4.05 → 4.69), so the fraction is always the worse trade. This is the contrast floor under the Two-Tier Text Rule: **on `jersey-deep` there is exactly one text colour, full cream.** If a surface needs a quiet tier, it may not be jersey-deep — `cream-quiet` is allowed only on `jersey-deep-dark` (~10.9:1) and ink (17.5:1). `<NumberDisplay tone="cream">` used to break this with a `text-cream/70` label; its label is now full cream, because its only consumer sits on jersey-deep. The day a deep-dark consumer appears, that label is too loud there and wants a tone-aware prop.
...
**The Token-Only Colour Rule (#2433).** Every colour comes from a design token above, never a bare Tailwind palette class (`text-red-600`, `bg-gray-50`, `border-purple-200`, …). The system has one paper, one ink, one green and a small set of named status tones on purpose — a raw palette class is always a colour this page-set doesn't have. Three separate efforts each rediscovered this independently (#2399, #2427, #2433) before it was written down here and enforced by lint (`no-restricted-syntax` in `apps/web/eslint.config.mjs`). The one sanctioned exception is a stories-only alignment ruler in `bg-fuchsia-500` — used **precisely because** fuchsia cannot be mistaken for a brand colour, each carrying its own `eslint-disable-next-line` rather than a blanket exemption. Test and spec files (`*.test.{ts,tsx}`, `*.spec.{ts,tsx}`) are outside the lint rule's reach, the same carve-out #2507/#2658 already made for the Motion Vocabulary bans — a fixture asserting against a raw class selector is not a shipped surface.
...
**The Type Ramp Freeze Rule.** The twelve steps above are the whole ramp — an arbitrary `text-[9px]`, `text-[.8rem]`, `text-[1.05rem]`, `text-[0.4em]`, `text-[14vw]`/`[12pt]`/`[50%]`/`[3ch]`, `text-[clamp(…)]`/`[calc(…)]`, `text-[length:var(--text-*)]` or its Tailwind v4 shorthand `text-(length:--text-*)` bypasses it outright, or bypasses it while looking token-driven (see "Reach for the class" above). `eslint.config.mjs` enforces this on every `className` written in `src/**/*.{ts,tsx}`: writing a fresh off-ramp literal there is now a lint error, not a style note. (An `.mdx` file's own prose, and a `@apply` line in CSS, are not `className` and are not reached by this rule.) It does not enforce it retroactively — the **314** literals that already existed across **87** files the day the rule landed are frozen, not fixed, via ESLint's native bulk-suppressions feature (`--suppress-rule kcvv/no-off-ramp-font-size`), snapshotted in the committed `apps/web/eslint-suppressions.json`. That count is the drain metric: ordinary `pnpm lint` (CI included) passes today only because those exact call sites are suppressed, and it starts failing the moment anyone adds a new one.

The rule runs under its **own rule ID**, `kcvv/no-off-ramp-font-size` (a local plugin wrapping the core `no-restricted-syntax` engine), rather than sharing the `no-restricted-syntax` array the Motion and Colour rules use. ESLint's native suppressions freeze a rule ID's _total count per file_, not each selector inside it — on a shared ID, swapping one frozen `text-[11px]` for a brand-new `duration-700` in the same file would have left the count unchanged and shipped clean. A dedicated ID makes that swap what it is: a new, unsuppressed violation, on a rule of its own.

Draining a call site — replacing the literal with its token — makes that file's suppression **stale, not silently fine**: `lint`, `lint-staged` and CI all start exiting non-zero the moment a suppressed violation stops occurring, by design (the whole "only shrinks" guarantee, not a bug to route around). Run `pnpm --filter @kcvv/web lint:prune` (`eslint . --prune-suppressions`) **in the same commit** as the drain to remove the stale entry — there is no hand-rolled allowlist to fall out of sync, only this one command. This ticket froze **font-size literals only**: `leading-*` line-height literals are a separate, not-yet-built rule (#2666) and are untouched here. Decided and built by [#2418](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2418).
...
**The Two-Line Hero Rule.** `leading-hero` (`--leading-hero: 0.85`) is a ramp step, not a `--text-*` step — it carries no font-size of its own, so it composes with whichever display step a two-line hero's own lines already use. Reserved for heroes that are a **fixed, structural** two-line composition — always exactly two lines regardless of content, never a single string that happens to wrap (`<PlayerHero>` / `<StaffHero>`'s first-name-over-last-name rhythm). **Never the default**, and never applied to a headline whose line count depends on content length or viewport (an `<EditorialHero>` title can render 1–3 lines depending on what an editor typed; there is no CSS way to detect the rendered line count, so a step this tight cannot govern it). The top line — the one with a kicker or jersey number sitting above it — takes `leading-hero-lead` (`@layer components`, globals.css), which bundles the `padding-top` its ascender clearance needs so the guard travels with the class rather than being re-typed (and sometimes forgotten) at every call site; the second line takes bare `leading-hero` on purpose, since it stacks tight against the first line's own descenders and padding there would work against the "one object" effect this step exists for (D14/Y1, decision-sheet §8, #2617).

**The Hanging Mark Rule.** A pull quote's opening mark (`<QuoteMark hang>`) hangs outside the card's padding via a negative `text-indent`, applied unconditionally in every engine — so the card's actual reading column (the blockquote body) keeps a straight left edge instead of the mark's own ink notching into it. Not gated behind `@supports (hanging-punctuation: first)`: the glyph is U+201D (Unicode category Pf), and WebKit's opening-punctuation check does not treat Pf as an opening character, so the native property is a no-op on this specific glyph in Safari — gating the fallback on it would have left Safari with no hang at all. `hang` is opt-in and off by default on `<QuoteMark>` itself (only `<PullQuote>` passes it): the pull is sized for `<TapedCard padding="lg">`, and a tighter wrapper collides the glyph into its neighbours (D14/Y6, decision-sheet §8, #2617).
...
**The Subject/Tag Rule.** A number takes its family from its **role**, never from its datum kind and never from its surface. A number that **is** the surface's subject — the match-detail hero score, a `<NumberDisplay>` stat on `/club/ultras` — keeps its display face (`font-display-big`). A number that **annotates** something else — a score in a fixture row, a minute beside a scorer, a standings cell, a day-of-month stub — is a tag and renders `font-mono`. **No `font-variant-numeric` utility is ever written in the site's own CSS/class-list surface, on either register** (the satori-rendered `/share` card is a separate renderer the class-list guard cannot reach and is exempted — see its own note). `tabular-nums` is inert on every family this site uses (measured on the served kit: nine distinct per-digit widths, byte-identical with and without the class — the kit ships no `tnum` feature at all), and `lining-nums` — the class this rule replaces — changes digit widths on both display faces without ever equalising them, so it never delivered a true tabular set either; only a genuinely monospaced face aligns a column, by construction. The default figure set on every Freight face already **is** oldstyle, so a subject number needs no class to get it. Decided by [#2516](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2516), built by [#2579](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2579), superseding [#2610](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2610)'s `lining-nums` fix.

**The one named exception: the lineup row's shirt number.** `MatchLineup.tsx`'s shirt-number chip looks exactly like rule 1's own "a shirt number beside a name" tag example, and reads as a bug if this rule is taken as absolute — it isn't. [#2621](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2621) ruled that a shirt number **in a lineup row specifically** is an identity mark, not a datum being tagged, and set it in the display register (black italic) on that basis. [#3029](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3029) is the owner's resolution confirming #2621 governs there, over rule 1's general tag example. **Scoped narrowly:** the squad-card shirt number (`PlayerCard.tsx`) is unaffected and stays mono — #2621 scopes itself to lineups only, and nowhere else. Do not extend this exception by analogy to any other "beside a name" site; it is one named surface, ruled once, not a second rule. **Accepted cost:** mono is the widest of the four faces, so a mono scoreline (e.g. `<TeamAgendaRow>`) eats into the width its neighbouring club names get, and long names truncate sooner than a display-face score would have allowed — shown on the prototype and accepted by the owner rather than missed.
...
**The Quiet Column Rule.** An article's 680px reading column is quiet by default: the page's character lives in the hero, the metadata bar, `EndMark`, the credits, the call-to-action band and the related row — not in ornaments scattered through the text. Inside the column an editor has exactly three tools, placed where the story turns, never by count: the star heading (`Tussentitel (groene divider)` → `<QASectionDivider>`), the quote card (`Citaat` → `<PullQuote>`) and photos. There is no frequency rule, and the body's `accent` decorator and the striped seam stay out of the editor's menu — drawn on a live article, they read as noise, and a seam announces a new section rather than a paragraph break. Measured when decided: of the last 9 articles, the 4 interviews were almost entirely `qaBlock`s and 4 of the 5 announcements were 80–250 words, so "one piece per 400 words" would have asked for nothing. Decided on [#2526](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2526).
...
Applied narrowly, not as a blanket 44px-everywhere pass: growing every link to 44px would add roughly 320px to the phone footer (19 rows × ~17px) and 5px to the desktop header against its 65px `--sticky-header-h` ceiling, to fix controls that already work — since #2394, every text link already clears 24px (footer 27, desktop nav 39, up-link chip 32, `Privacy`/`Cookie-instellingen` 24), and the phone header's icons are already 44 × 44 (`SiteHeader.tsx`: menu `min-h-11 min-w-11`, search `h-11 w-11`). The rule closed the three controls that were still icon-only and under 24px: `ArticleMetadata`'s share control (merged from two 16px icons into one labelled `Delen` button, 44px tall) and `SiteHeader`'s desktop `Zoeken` link (`h-11 w-11`, icon size unchanged).

**A hit area may only spill into space its own container already owns — its gap or its padding — never onto a neighbour.** This is the mechanism, not an alternative to it: growing a control past its visible box always needs a matching negative margin to cancel the added space back out of the layout (`SiteHeader`'s desktop nav links, `-my-2 py-2`, #2394; the wordmark, `-my-1 py-1`; `HubSearch`'s clear button, `-m-1.5 p-1.5`), and the _only_ question is whether the surrounding container already reserves enough of its own gap or padding for that cancelled-out overhang to land in without touching anything else. `ArticleMetadata`'s `Delen` button follows the same idiom (`-my-3.5 py-3.5`): the metadata row's own `gap-y-3.5` absorbs the overhang above the button's line when the bar wraps (so it reaches the facts line's edge without crossing it), and the bar's own `py-3.5` absorbs it on both sides otherwise (so it reaches the bar's own border without crossing into the hero or the article body next to it). `SiteHeader`'s desktop `Zoeken` link is the same shape read the other way: `-mx-[13px]` cancels the width `h-11 w-11` (44×44) adds around the 18px icon, so the link's contribution to the row stays exactly what it was before the hit area existed, and the nav→icon and icon→CTA gaps stay at the grid's own gap instead of growing unevenly. Decided on [#2529](https://github.com/soniCaH/www.kcvvelewijt.be/issues/2529).

**A control whose visible box carries its own border cannot use that idiom** — padding grows the border along with the hit area. It takes the `hit-area` utility (`apps/web/src/app/globals.css`) instead: the control becomes `position: relative` and a transparent `::before` at `inset: calc(50% - 22px)` supplies a 44 × 44 hit area around any box up to 44px, while the visible box stays exactly as drawn. The same containment rule applies — the overhang lands in the container's own gap or padding, no ancestor may clip it, and it never reaches a neighbour. `ContactCard`'s `E-mail` / `Bel` actions (40px), `OrganigramExplorer`'s sibling arrows (28px) and `MatchStripView`'s result/fixture toggle (36px) use it. `HubSearch`'s clear button has no border and could have grown its pull-back padding instead; it takes the same utility on top of its existing `-m-1.5 p-1.5` (which now only sizes the visible 32px hero / 28px nav box) so every sub-44 icon control answers to one idiom. Its text field gives back an end margin (4px hero, 6px nav) while the button shows, so the hit area never covers the field. `CalendarWidget`'s period arrows take a visible `h-11 w-11` at every width instead — the owner's pick after the invisible variant was measured poking 6px outside the toolbar row on production. Decided on [#3076](https://github.com/soniCaH/www.kcvvelewijt.be/issues/3076).
...
- Pills are sharp-cornered with 0.5rem / 0.25rem padding. The `MonoLabel` jersey-deep pill (`pill-jersey-deep`) still uses pure white text, not cream — it was adopted when cream on the old `#008755` was the weaker pairing, and on today's darker green full cream would clear comfortably too, so the white is inherited rather than required. **#2421 left it as-is deliberately**: the variant renders across several surfaces (`EventHero`, `EditorialHubCard`, `StaffHero`, `StaffRoles`), so it stays the design-system owner's call rather than an audit's. `event-type-style.ts`'s `EVENT_TYPE_FILL` map is **not** a primitive — it's a feature map shared with one: its `Clubevent` value feeds `<FilterTabs>`'s selected-tab fill, a primitive's selected-state colour, so #2421 parked it there too. Its other, non-tab consumers (`TicketStub`'s tear-off badge, `CalendarMonth`/`CalendarWeek`'s day chips, `calendar-tags.tsx`) are white on jersey-deep for no reason of their own — they inherit the one shared map value — and are parked alongside the same `FilterTabs` decision rather than reconciled individually. The handful of **feature-component** white-on-jersey-deep sites that had their own, independent white (`TeamFlagship`, `TeamAgendaRow`, `OrgPersonCard`, `FeaturedSponsorCard`, `QuestionCard`, `CalendarSubscribePanel`, `UpcomingMatchesClient`'s home badge, `EditorialHero`'s credit chip) were reconciled to cream in #2421. Fractional cream, however, is never the answer here: see The Whole-Cream Rule.
...
- The bar itself pins at `--sticky-header-h`, `bg-cream-deep`, `border-b-2`; a trailing slot (`<HubSearch variant="nav">` on `/hulp`) takes the chip's exact paper weight **and is never taller than the chip**, so the bar reads as one row of one kind of object. Both halves are load-bearing and only the first was kept until #2821: the slot matched the chip's 1px border and 1px shadow but sat at 41px against the chip's 33px, because it pairs 13px type with `py-2` where the chip pairs 11px with `py-1.5`. A trailing slot that outgrows the chip does not just read wrong — it makes the whole bar change height when it appears mid-scroll, which `useSectionNav` must then re-publish as `scroll-padding-top` and `useHashLandingCorrection` must correct an in-flight hash landing against. The slot keeps its 13px type (it is a field, not a label) and buys the height back out of its padding: `SECTION_NAV_TRAILING_SLOT_PADDING` in `section-nav.ts` carries the arithmetic, deliberately beside `SECTION_NAV_CHIP_BASE_CLASSES` rather than in the consumer, because the rule is a comparison between the two. The row does **not** wrap either: the slot carries `min-w-0` and squeezes instead, so a narrow viewport cannot push it onto a second line and take the bar from 52px to 106px.
...
**The Document Has Nothing To Show Rule.** A document is one of two subject kinds with no artefact, and that is not a gap — there is no illustration for "a piece of writing". The hatch is correct for exactly this case, which is why it stays `<NewsCard>`'s own default rather than becoming a fifth artefact. (The other artefact-less kind, a nav tile, is the "No Filler Photo, Ever Rule" below — it falls back to the flat green panel + glyph, never the hatch, because a destination isn't a document either.)
...
**No Filler Photo, Ever Rule (#2965).** This is the render-time Writer Rule applied to imagery, for the second artefact-less subject kind, the nav tile (see The Document Has Nothing To Show Rule, above). The recipe lives on the primitive, not per-consumer: any `<EditorialHubCard variant="nav">` that receives an `imageUrl` prop shows the photo; omit it (the default) and the card keeps the flat `bg-jersey-deep` + 40px glyph, unchanged — never by switching a call site to `variant="news"` instead. A generic stand-in photo is never substituted for a missing one — a photo on "Blessure of medisch attest?" would read worse than an honest icon, the same trap the Phase 4.5 typographic cards fell into ("they don't shine at all"). Present renders the photo at 16:9 with the same newsprint treatment every cover photo gets (see above), behind a flat `jersey-deep-dark/62` scrim, with the pill and glyph on top. The scrim is deliberately flat, not `--gradient-jersey-deep-overlay` — the mock specified a flat tone for this tile, so that gradient token stays unclaimed by any shipped photographic surface rather than overridden by this one; the next jersey-deep photo surface should read this note before picking one or the other. The photo is decorative (`alt=""`) — see § What an Image Says for why a cover under its own labelled link needs none. On `/jeugd`, the hardcoded `NAV_CARDS` fallback points `image` at `apps/web/public/images/jeugd/` — chrome, not editorial content, so an editor deleting an article can't break a nav tile — while a Sanity-authored nav card (`editorialCards`) carries its own `image` field through the CMS instead.
...
**The Silence Is An Answer Rule (#2470/#2580).** A client-initiated failure — a search, a submit, a copy-to-clipboard — gets a visible notice only when nothing else already recovers the visitor's intent. The test: does anything else on screen still let the visitor finish what they were doing? If yes, render nothing; the silence **is** the answer, not a gap to fill. Four sites are ratified under this test, by name, and stay silent on purpose: `/hulp`'s `HubSearch` (falls back to keyword search — the PRD floor, #2057 decision 7o8), `/zoeken`'s semantic augment (`useSemanticAugment` — the lexical results below still serve), `/nieuws/[slug]`'s `navigator.share` rejection (`ArticleMetadata` — a dismissed share sheet is not a failure, and the sibling Facebook link is the explicit alternative), and `/zoeken`'s own failed lexical search when the semantic lane already answered (`SearchInterface` — a high-confidence "Slim antwoord" card recovers the visitor's intent, so the failure is suppressed but still counted via a `search_failed` analytics event, #2824). A fifth site follows the same test, not the shape of these four.
