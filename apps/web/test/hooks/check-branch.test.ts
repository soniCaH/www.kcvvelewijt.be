...
const DENIED: Case[] = [
  ...
  {
    name: 'a quoted argument before the commit — pnpm turbo run … --filter="@kcvv/web" && git commit',
    command:
      'pnpm turbo run lint type-check test build --filter="@kcvv/web" && git commit -m x',
    cwd: "{MAIN}",
  },
  {
