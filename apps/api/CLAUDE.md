...
- **A gate token is pacing, not observability — incident alerting still does not cover the scheduled paths.** `gate.reportOutcome` (the call that opens/closes a PSD incident and fires the Slack ping from #2329) is invoked from the `TypedKvCache` refresh path in `cache/kv-cache.ts` and nowhere else. The nightly sync touches `KvCacheService` only for `increment()`, so it never reaches one. **A week of 02:00 sync failures against a 429-ing PSD produces a `console.error` in the awaited `scheduled()` branch and nothing else** (the sync is `await`ed directly, not fired via `ctx.waitUntil` — #2900) — no incident, no Slack, no drift nudge (drift is a cache-key concept and the sync writes no cached value). Do not read the pacing rule above as covering this. Closing the gap is **#2870**, and it starts with a decision, not wiring: whether a sync failure should open the same incident the read path opens. Note it cannot simply reuse `reportOutcome` — the read path reads `incidentOpen` back to escalate its serve-stale logs `WARN→ERROR`, so a failing 02:00 cron would make request-path logs shout while the site serves fine — a sync that fails while the site serves fine is not the same event as PSD being down for visitors.
